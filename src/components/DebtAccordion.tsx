...
export const DebtAccordion: React.FC<DebtAccordionProps> = memo(({
  ...
  const debtItems = useMemo(() => 
    sortedDebts.map((debt, index) => {
      ...
      return { debt, index, totalPaid, remaining, progress, daysLeft, isWarning, warningText, nextPaymentDate, isCompleted };
    }), [sortedDebts]
  );

  // Aktif ve tamamlanmış borçları ayır
  const activeDebts = debtItems.filter(item => !item.isCompleted);
  const completedDebts = debtItems.filter(item => item.isCompleted);
    ...
    <div className="space-y-6">
      ...
      {activeDebts.length > 0 && (
        <Accordion type="multiple" className="space-y-4">
          {activeDebts.map(({ debt, index, totalPaid, remaining, progress, daysLeft, isWarning, warningText, nextPaymentDate, isCompleted }) => (
                        ...
                        {debt.payments.map((payment: any) => (
                          <div key={payment.id} className="flex justify-between items-center text-xs bg-secondary/30 p-2 rounded">
                            ...
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                if (confirm("Bu ödemeyi geri almak istediğinizden emin misiniz?")) {
      ...
      {completedDebts.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <span className="text-sm font-medium px-3">Tamamlandı ({completedDebts.length})</span>
            <div className="h-px bg-border flex-1"></div>
          </div>
          
          <Accordion type="multiple" className="space-y-4">
            {completedDebts.map(({ debt, index, totalPaid, remaining, progress, daysLeft, isWarning, warningText, nextPaymentDate, isCompleted }) => (
              <AccordionItem key={debt.id} value={debt.id} className="border rounded-lg bg-muted/30 opacity-75">
                <AccordionTrigger className="hover:no-underline p-0">
                  <div className="w-full">
                    <div className="bg-muted/50 px-4 py-2 border-b">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm text-muted-foreground">{formatCurrency(debt.totalAmount)}</span>
                      </div>
                    </div>
                  </div>
                </AccordionTrigger>
